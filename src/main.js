...
let manageClaudeHooksAutomatically = false;
...
function syncRendererStateAfterLoad({ includeStartupRecovery = true } = {}) {
  ...
  if (doNotDisturb) sendToRenderer("dnd-change", true);
  if (_mini.getMiniMode()) {
...
function playPetHappy(source = "click") {
  return autonomousBehavior.playTransient("attention", 2000, { source, manual: true });
}

function teasePet(source = "menu") {
  return autonomousBehavior.tease(source);
}

function enableStandaloneDoNotDisturb() {
  enableDoNotDisturb();
  autonomousBehavior.setPaused(true);
}

function disableStandaloneDoNotDisturb() {
  disableDoNotDisturb();
  autonomousBehavior.setPaused(false);
}
...
const _menuCtx = {
  ...
  teasePet: (source) => teasePet(source),
  ...
  enableDoNotDisturb: () => enableStandaloneDoNotDisturb(),
  disableDoNotDisturb: () => disableStandaloneDoNotDisturb(),
...
const MENU_AFFECTING_KEYS = new Set([
  "lang", "soundMuted", "openAtLogin", "showTray", "showDock", "theme", "size",
]);
...
ipcMain.handle("settings:get-about-info", () => {
  ...
  return {
    ...
    repoUrl: "https://github.com/bestxiangest/clawd-buddy",
    license: "MIT",
...
function createWindow() {
  ...
  ipcMain.on("pet-single-click", () => {
    playPetHappy("single-click");
  });
  ...
  startMainTick();
  autonomousBehavior.setPaused(doNotDisturb);
...
const createAutonomousBehavior = require("./autonomous-behavior");
const autonomousBehavior = createAutonomousBehavior({
  getCurrentState: () => _state.getCurrentState(),
  getMiniMode: () => _mini.getMiniMode(),
  isMiniTransitioning: () => _mini.getMiniTransitioning(),
  isBlocked: () => petHidden || dragLocked,
  resetIdleTimer: () => resetIdleTimer(),
  playState: (state) => {
    if (_mini.getMiniMode() && state === "working" && _state.getCurrentState() === "mini-idle") {
      applyState("working");
      return;
    }
    applyState(state);
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    // Standalone build: no external agent monitors.
